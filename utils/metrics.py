...
Based on SPOT repository:
https://github.com/gkakogeorgiou/spot/blob/master/ocl_metrics.py

Which was based on OCLF library:
...
class ARIMetric(torchmetrics.Metric):
    ...
    def update(
        ...
        assert torch.all(
            target.sum(dim=-1) < 2
        ), "Issues with target format, mask non-exclusive"
...
class BestOverlapObjectRecoveryMetric(UnsupervisedMaskIoUMetric):
    def __init__(self, **kwargs):
        super().__init__(
            matching="best_overlap", compute_discovery_fraction=True, **kwargs
        )
...
def unsupervised_mask_iou(
    ...
    if matching == "hungarian":
        ...
        true_idxs = torch.arange(pairwise_iou.shape[1]).to(non_empty_gt.device)[
            non_empty_gt
        ]
    else:
...
def tensor_to_one_hot(tensor: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    one_hot = max_idxs == torch.arange(tensor.shape[dim], device=tensor.device).view(
        *shape
    )
...
def adjusted_rand_index(
    ...
    expected_rindex = (
        aindex * bindex / torch.clamp(n_fg_points * (n_fg_points - 1), min=1)
    )
...
def _remap_one_hot_mask(
    ...
    mask = torch.nn.functional.one_hot(
        mask_dense.to(torch.long), num_classes=n_new_classes + 1
    )
