...
class MOVIe(Dataset):
    def __init__(self, file_root, sequence_length, resolution, split="train"):
        ...
        assert sequence_length == 1 or sequence_length == 24, "sequence_length must be 1 (image) or 24 (full sequence)"
        ...
        for video in sorted(os.listdir(path)):
            ...
            frames = sorted([os.path.join(video_path, frame) for frame in os.listdir(video_path)])
